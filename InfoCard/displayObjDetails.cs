    ...
    public void parseFields()
    {
        ...
        foreach (string line in lines)
        {
            ...
            if (line[0] == '*')//Custom Field
            {
                string[] headerFields = line.Split('|');
                headerFields[0] = headerFields[0].Substring(1);
                uiObject.createCustom(data, int.Parse(data.getData(headerFields[1])));
                continue;
            }
        ...
        foreach (KeyValuePair<string, string> entry in comparisons)
        {
            ...
            if (entry.Key == data.getData(comparisonVal) || entry.Key == "none")
            {
                ...
                foreach (string line in nlines)
                {
                    ...
                    if(fields.Length < 2)
                    {
                        uiObject.createDescText(data.getData(fieldName,idx));
                    }
                    else
                    {
                        ...
                        if(secVals.Length <=1)
                        {
                            uiObject.createText(fields[0], data.getData(fieldName, idx),parent);
                        }
                        else
                        {
                            uiObject.createText(fields[0], data.getData(secVals[0], idx) + "/" + data.getData(secVals[1], idx),parent);
                        }
                    }
